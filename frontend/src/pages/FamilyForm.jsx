        ...
        <Modal open={open} onOpenChange={onOpenChange} title="New Family">
            <form onSubmit={submit} className="flex flex-col gap-4">
                <div>
                    ...
                    <input data-testid={FAMILY_FORM.name} required value={form.family_name} onChange={(e) => set("family_name", e.target.value)} className="eat-input mt-1" placeholder="Hernandez" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="eat-label">Guardian Name</label>
                        <input data-testid={FAMILY_FORM.guardianName} required value={form.guardian_name} onChange={(e) => set("guardian_name", e.target.value)} className="eat-input mt-1" />
                    </div>
                    <div>
                        <label className="eat-label">Guardian Phone</label>
                        <input data-testid={FAMILY_FORM.guardianPhone} required value={form.guardian_phone} onChange={(e) => set("guardian_phone", e.target.value)} className="eat-input mt-1" />
                    </div>
                </div>
                <div>
                    <label className="eat-label">Guardian Email</label>
                    <input data-testid={FAMILY_FORM.guardianEmail} type="email" required value={form.guardian_email} onChange={(e) => set("guardian_email", e.target.value)} className="eat-input mt-1" />
                </div>
                ...
                <button data-testid={FAMILY_FORM.submit} type="submit" className="eat-btn-primary w-full mt-2">Create Family</button>
            </form>
        </Modal>
    );
}
